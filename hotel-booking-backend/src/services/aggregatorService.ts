...
/** Merge image URL lists in priority order; no count cap or placeholder URLs. */
function mergeImagesByPriority(sources: (string[] | undefined | null)[]): string[] {
...
const SYNTHETIC_ROOM_NAMES = new Set([
  "standard room",
  "deluxe room",
  "executive suite",
  "presidential suite",
...
/** True when rooms were generated by buildRooms / booking Path B — not from API inventory. */
function looksSyntheticRooms(rooms: RoomType[]): boolean {
  if (rooms.length === 0) return false;
  return rooms.every((r) => SYNTHETIC_ROOM_NAMES.has(r.type.toLowerCase().trim()));
}

/**
 * Pick final room list: prefer real Expedia inventory over synthetic Booking fallbacks.
 */
function resolveFinalRooms(
  bookingMapped: RoomType[],
  expediaMapped: RoomType[],
  fallback: () => RoomType[]
): RoomType[] {
  const hasExpedia = expediaMapped.length > 0;
  const hasBooking = bookingMapped.length > 0;
  const bookingSynthetic = looksSyntheticRooms(bookingMapped);

  if (hasExpedia && (!hasBooking || bookingSynthetic)) {
    return expediaMapped;
  }
  if (hasExpedia && hasBooking) {
    return mergeRoomLists(bookingMapped, expediaMapped);
  }
  if (hasBooking) return bookingMapped;
  return fallback();
}

/** Merge Booking + Expedia rooms; Booking wins on name conflict; fill missing images from Expedia. */
function mergeRoomLists(bookingRooms: RoomType[], expediaRooms: RoomType[]): RoomType[] {
  if (!bookingRooms.length) return expediaRooms;
  if (!expediaRooms.length) return bookingRooms;

  const expByName = new Map(
    expediaRooms.map((r) => [r.type.toLowerCase().trim(), r])
  );
  const merged = bookingRooms.map((b) => {
    const ex = expByName.get(b.type.toLowerCase().trim());
    if (!ex) return b;
    return {
      ...b,
      imageUrl: b.imageUrl || ex.imageUrl || "",
      size: b.size !== "—" ? b.size : ex.size,
    };
  });
  for (const ex of expediaRooms) {
    const key = ex.type.toLowerCase().trim();
    if (!merged.some((m) => m.type.toLowerCase().trim() === key)) {
      merged.push(ex);
    }
  }
  return merged;
...
export async function enrichDBHotel(dbHotel: any): Promise<EnrichedHotel> {
  ...
  const finalImages  = mergeImagesByPriority([baseImages, googleImages]);
...
export async function getEnrichedHotelDetails(
  ...
  const googleImages = (google?.photos ?? []).map((p) => p.url).filter(Boolean);
  const finalImages  = mergeImagesByPriority([googleImages, rapidImages]);
...
export async function enrichHotelData(baseHotel: any): Promise<EnrichedWithExtra> {
  ...
  // ── Step 4: IMAGES — owner > Expedia > Google > other providers ─────────
  const isDbHotel = String(baseHotel?.source ?? "") === "db"
    || (baseHotel?._id && !String(baseHotel._id).startsWith("booking_"));
  const ownerImages = isDbHotel && Array.isArray(baseHotel?.imageUrls)
    ? baseHotel.imageUrls
    : [];
  const providerImages = !isDbHotel && Array.isArray(baseHotel?.imageUrls)
    ? baseHotel.imageUrls
    : [];
  const images = mergeImagesByPriority([
    ownerImages,
    expNorm.images,
    (googleDetails?.photos ?? []).map((p: any) => p.url),
    providerImages,
  ]);
  ...
  const bookingMapped = bookExt.rooms.map((r, i) =>
    mapExtractedRoom({ ...r, id: r.id ?? `booking-${i}` })
  );
  const expediaMapped = expNorm.rooms.map((r, i) =>
    mapExpediaRoom({
      id:                 r.id ?? `expedia-${i}`,
      name:               r.name,
      maxGuests:          r.maxGuests,
  ...
  const finalRooms = resolveFinalRooms(
    bookingMapped,
    expediaMapped,
    () => buildRooms(
  ...
  const propertyDescription =
    String(baseHotel?.description ?? "").trim() ||
    expNorm.description.trim() ||
    "";
